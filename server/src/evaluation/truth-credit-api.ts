...
async function runTruthCreditApiCase(params: {
  ...
  const expectedTargetReasonIntegrityScore = expectedTargetReasonIntegrityTotalCount > 0
    ? expectedTargetReasonIntegrityChecks.filter(Boolean).length / expectedTargetReasonIntegrityTotalCount
    : 1;
  ...
  const expectedTargetReasonIntegrityGap = expectedTargetReasonIntegrityTotalCount - expectedTargetReasonIntegrityChecks.filter(Boolean).length;
